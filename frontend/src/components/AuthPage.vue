      ...
      <div v-else class="auth-content fade-in">
        ...
        <div class="auth-providers">
          ...
          <template v-if="Capacitor.isNativePlatform()">
            <a href="#" @click.prevent="loginWithTelegramNative" class="provider-btn telegram-btn">
...
async function loginWithTelegramNative() {
  // Генерируем уникальный state для этой сессии авторизации
  const state = crypto.randomUUID().replace(/-/g, '');
  
  try {
    // Открываем страницу с TG виджетом, передавая state
    await Browser.open({ 
      url: `https://ai.bralin.kz/auth/mobile_login.php?state=${state}`,
  ...
  // Начинаем опрашивать сервер — появился ли JWT для нашего state
  ...
  tgPollInterval = setInterval(async () => {
    ...
    try {
      const res = await fetch(`/api/check_mobile_token.php?state=${state}`);
      const data = await res.json();
      if (data.ok && data.token) {
        clearInterval(tgPollInterval);
        localStorage.setItem('nc_token', data.token);
      ...
      // Сетевая ошибка — продолжаем опрашивать
    }
  }, 1000);
}
